  ...
  const { data, error } = await supabase.from("contact_inquiries").insert([
    {
      ...
      subject: "Custom Dinnerware Set Inquiry",
      message: "Hi, I love your handcrafted tableware! Can I order a set of 8 plates in matte olive?",
    }
  ]).select();

  console.log("Inserted test inquiry:", data, "error:", error);
}
